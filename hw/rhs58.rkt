...
(define (all_q L)
  (foldl (lambda (a b) (if (equal? 'q a) b #f)) #t L)
)
...
(define (at_least_one_q_v2 L)
  (foldl (lambda (a b) (if (equal? 'q a) #t b)) #f L)
)
...
(define (odd_amt_q_v2 L)
  (foldl (lambda (a b) (if (equal? a 'q) (if b #f #t) (if b #t #f))) #f L)
)
