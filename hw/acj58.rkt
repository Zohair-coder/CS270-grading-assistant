...
(define (andlist L)
  (foldr (λ (x y) (if x (if y #t #f) #f)) #t L)
)
...
(define (orlist L)
  (foldr (λ (x y) (if x #t (if y #t #f))) #f L)
)
...
(define (all_q L)
  (if(null? L)
     #t
     (if (equal? 'q (first L)) (all_q (rest L)) #f)
  )
)
...
(define (at_least_one_q L)
  (if (null? L)
      #f
      (if (equal? 'q (first L)) #t (at_least_one_q (rest L)))
  )
)
...
(define (exactly_one_q L)
  (if (null? L)
      #f
      (if (equal? 'q (first L)) (not (at_least_one_q (rest L))) (exactly_one_q (rest L)))
  )
)
...
(define (odd_amt_q L)
  (if (null? L)
      #f
      (if (equal? 'q (first L)) (not (odd_amt_q (rest L))) (odd_amt_q (rest L)))
  )
)
