...
(define (all_q L)
  ...
  ;(if (equal? (if(null? L) null(cons(first L)(all_q(rest L)))) '(q))
  ;Complete this function definition.
)
...
(define (at_least_one_q_v2 L)
  (orlist(map (lambda (x) (equal? 'q x)) L));Complete this function definition.
)
...
(define (exactly_one_q L)
  (cond
    [(null? L) #f]
    [(equal? 'q (first L)) (andlist(map(lambda(x)(if (equal? 'q x) #f #t)) (rest L)))]
    [else (exactly_one_q (rest L))]
  ;0Complete this function definition.
))
...
(define (odd_amt_q L)
  (cond
    [(null? L) #f]
    [(equal? 'q (first L))(xori(equal? 'q (first L)) (odd_amt_q(rest L)))]
