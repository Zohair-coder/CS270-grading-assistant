...
(define (andlist L)
  (foldl (λ (x y) (if y (if x #t #f) #f)) #t L) 
)
...
(define (orlist L)
  (foldl (λ (x y) (if y #t (if x #t #f))) #f L)
)
...
(define (all_q_v2 L)
  (foldl (λ (x y) (if y (if (equal? x 'q) #t #f) #f)) #t L) 
)
...
(define (at_least_one_q_v2 L)
  (foldl (λ (x y) (if y #t (if (equal? x 'q) #t #f))) #f L)
)
...
(define (odd_amt_q_v2 L)
  (foldl (λ (x y) (if y (if (equal? x 'q) #f #t) (if (equal? x 'q) #t #f))) #f L) 
  )
