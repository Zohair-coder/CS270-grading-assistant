...
(define (all_q L)
  (if (null? L) #t (andi (equal? (first L) 'q) (all_q (rest L))))
)
...
(define (all_q_v2 L)
  (andlist (map (lambda (a) (equal? a 'q)) L))
)
...
(define (at_least_one_q L)
  (if (null? L) #f (ori (equal? (first L) 'q) (at_least_one_q (rest L))))
)
...
(define (at_least_one_q_v2 L)
  (orlist (map (lambda (a) (equal? a 'q)) L))
)
...
(define (exactly_one_q L)
  null;(if (null? L) #f (not (iffi (equal? (first L) 'q) (exactly_one_q (rest L)))))
)
...
(define (odd_amt_q L)
  (if (< (length L) 3) (not (iffi (equal? (first L) 'q) ((equal? (second L) 'q)))) (iffi (equal? (first L) 'q) (odd_amt_q (rest L))))
)
