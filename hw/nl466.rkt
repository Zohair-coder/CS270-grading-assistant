...
(define (all_q L)
  (if (empty? L) #t (if (equal? (list (first L)) '(q)) (all_q (rest L)) #f))
)
...
(define (all_q_v2 L)
  (foldl equal? #t (map (lambda (letter) (equal? '(q) (list letter))) L))
)
...
(define (at_least_one_q L)
  (if (empty? L) #f (if (equal? (list (first L)) '(q)) #t (at_least_one_q (rest L))))
)
...
(define (at_least_one_q_v2 L)
  (ormap (lambda (letter) (equal? '(q) (list letter))) L)
)
...
(define (exactly_one_q L)
  (if (empty? L) #f (if (equal? (list (first L)) '(q)) (if (at_least_one_q (rest L)) #f #t) (exactly_one_q (rest L))))
)
...
(define (odd_amt_q L)
  (if (empty? L) #f (if (equal? (list (first L)) '(q)) (if (odd_amt_q (rest L)) #f #t) (odd_amt_q (rest L))))
)
