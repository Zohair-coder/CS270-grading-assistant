...
(define-test-suite testxori
  ...
  (test-equal? ""
    (xori #f #t) #t)
  (test-equal? ""
    (xori #t #f) #t) ;Doesn't work for this
  (test-equal? ""
    (xori #t #t) #f))
...
;  (if (null? L) #f
;      (if (ori (equal? (first L) 'q) (equal? (rest L) 'q)) #f
...
(define (odd_amt_q_v2 L)
  (foldl xori #f (map (λ (x) (equal? x 'q)) L)))
