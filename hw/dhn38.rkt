...
(define (all_q_v2 L)
  (if (null? L) #t (andlist (map (λ (x) (if (equal? 'q x) #t #f)) L)))
)
...
(define (at_least_one_q_v2 L)
  (if (null? L) #f (orlist (map (λ (x) (if (equal? x 'q) #t #f)) L)))
)
...
(define (exactly_one_q L)
  (cond
    [(null? L) #f]
    [(equal? 'q (first L)) (andlist (map (λ (x) (if (equal? x 'q) #f #t)) (rest L)))]
...
(define (odd_amt_q_v2 L)
  (if (null? L) #f (foldl xori #f (map (λ (x) (if (equal? 'q x) #t #f)) L)))
)
