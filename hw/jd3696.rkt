...
(define (all_q L)
  (if (null? L)
      'q
      (andi (equal? 'q (first L)) (all_q (rest L)))
      )
...
(define (exactly_one_q L)
  (if (null? L)
      #f
      (ori
       (andi (equal? (first L) 'q) (noti (at_least_one_q (rest L))))
       (andi (noti (equal? (first L) 'q)) (exactly_one_q (rest L)))
       )
      )
)
...
(define (odd_amt_q_v2 L)
  (foldl (lambda (x y) (xori (equal? x 'q) y)) #f L)
)
