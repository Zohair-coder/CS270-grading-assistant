...
(define (andlist L)
  (if (equal? L null)
      #t
      (foldr andi #t (map (lambda (x) (if (equal? x #t) #t #f)) L)))
)
...
(define (orlist L)
  (if (equal? L null)
      #f
      (foldr ori #f (map (lambda (x) (if (not(equal? x #f)) #t #f)) L)))
  )
...
(define (all_q_v2 L)
  (if (equal? L null)
      #t
      (andlist (map (lambda (x) (if (equal? x 'q) #t #f)) L))))
...
(define (at_least_one_q_v2 L)
  (if (equal? L null)
      #f
      (orlist (map (lambda (x) (if (equal? x 'q) #t #f)) L))))
...
(define (exactly_one_q L)
   (cond
    [(equal? L null) #f]
    [(equal? (first L) 'q) (if (at_least_one_q (rest L)) #f #t)]
...
(define (odd_amt_q L)
  (cond
    [(equal? L null) #f]
    [(equal? (first L) 'q)(not (odd_amt_q (rest L)))]
