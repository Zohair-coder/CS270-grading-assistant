...
4. Using If/Cond to p; Sara Beinish
#lang racket
...
(define (andlist L)
  (cond
    [(null? L) #t]
    [(first L) (foldl (lambda (x y) (andi x y)) #t L)]
    [else #f]
))
...
(define (all_q_v2 L)
  (zero? (foldl (lambda (x y) (+ (if (equal? x 'q) 0 1) y)) 0 L)))
...
(define (at_least_one_q L)
  (if (null? L) #f (if (equal? (first L) 'q) #t (at_least_one_q (rest L)))))
...
(define (at_least_one_q_v2 L)
    (if (zero? (foldl (lambda (x y) (+ (if (equal? x 'q) 1 0) y)) 0 L)) #f #t))
...
(define (exactly_one_q L)
  ;if first isnt q, hide first element then ask if the rest has exactly one q
  ;if first is q, make sure the rest doesn't have a q. if so return true
  
  (equal? (foldl (lambda (x y) (+ (if (equal? x 'q) 1 0) y)) 0 L) 1))
...
(define (odd_amt_q_v2 L)
  (if (zero? (remainder (foldl (lambda (x y) (+ (if (equal? x 'q) 1 0) y)) 0 L) 2)) #f #t)
)
...
(display grand_total)
(display "/100\n")ass the specific tests provided instead of following the instructions will always
result in a zero for that question.  Do not assume that simply because your code passes all the example
unit tests here then that means you get 100%; we will be testing with input beyond the examples.
The graders will still obey the input contracts.
...
(define (andlist L)
  (if (null? L) #t (foldl (lambda (x y) (andi x y)) (first L) L)))
...
;(define (orlist L)
;  (if (null? L) #f (foldl (lambda (x y) (ori x y)) (first L) (rest L))))
;here i wasn't sure if using ori is considered using a helper function so here's another version
;where it uses no other functions:

(define (orlist L)
  (if (null? L) #f (foldl (lambda (x y) (if x #t (if y #t #f))) (first L) (rest L))))
...
(define (all_q_v2 L)
  (if (null? L) #t (andlist (map (lambda (x) (equal? x 'q)) L))))
...
(define (at_least_one_q_v2 L)
  (if (null? L) #f (orlist (map (lambda (x) (equal? x 'q)) L))))
...
;if theres 1 q its true
;otherwise we dont know so give a smaller version of the list

(define (exactly_two_q L)
  (cond
    [(null? L) #f]
    [(equal? (first L) 'q) (exactly_one_q (rest L))]
    [else (exactly_two_q (rest L))]))

(define (odd_amt_q L)
  (cond
    [(or (not (at_least_one_q L)) (null? L)) #f] ;if its empty or has no q's its false (0 is even)
    [(exactly_one_q L) #t] ;if theres one its true (1 is odd)
    [(exactly_two_q L) #f] ;if theres two its false (2 is even)
    [(equal? (first L) 'q) (not (odd_amt_q (rest L)))] ;if there's more than 2 q's we don't know so if the first is equal to q, check if the rest is even
...
; Output: (odd_amt_q L) is true if and only if the amount of q's in L is odd.
(define (odd_amt_q_v2 L)
  (if (null? L) #f (foldl xori #f (map (lambda (x) (equal? x 'q)) L))))
