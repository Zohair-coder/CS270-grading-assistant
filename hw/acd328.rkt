...
(define (andlist L)
  (foldr (lambda(a b) (if a (if b #t #f) #f)) #t L)
)
...
(define (orlist L)
  (foldr (lambda(a b) (if a #t (if b #t #f))) #f L)
)
...
(define (exactly_one_q L)
   (cond
    [(null? L) #f]
    [(equal? (first L) 'q) (if (exactly_one_q (rest L)) #f #t)]
...
(define (odd_amt_q L)
     (cond
    [(null? L) #f]
    [(equal? (first L) 'q) (if (odd_amt_q (rest L)) #f #t)]
...
(define (odd_amt_q_v2 L)
  ;(foldr (lambda(a b) (if a #t (if b #t #f))) #f L)
  ;[(equal? (first L) 'q) (if (odd_amt_q (rest L)) #f #t)]
  (foldr (lambda (a b) (if (equal? a 'q) (if b #f #t) #t)) #f L)
