...
(define (iffi a b)
  (if (if a (if b #t #f) #t) (if b (if a #t #f) #t) #f)
)
...
(define (all_q_v2 L)
  (if (null? L) #t (if (equal? (first L) 'q) (andlist(rest L)) #f))
)
...
(define (at_least_one_q_v2 L)
  (if (null? L) #f (if (equal? (first L) 'q) #t (orlist (rest L))))
)
...
(define (exactly_one_q L)
  (if (null? L) #f (if (equal? (first L) 'q) #t (exactly_one_q(rest L))))
)
...
(define (odd_amt_q L)
  (if (null? L) #f (if(equal? 'q (first L)) (odd_amt_q (rest L)) (noti (odd_amt_q (rest L)))))
)
...
(define (odd_amt_q_v2 L)
  (if (null? L) #f (noti (odd_amt_q (rest L)))) 
)
