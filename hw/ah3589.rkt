...
(define (all_q_v2 L)
  ;(if (foldl equal? 'q L) #t #f) ;This passed seven tests
  
  (map (lambda (x) (if (equal? 'q x) #t #f)) L)
  (andlist L)
  ;This only passed 4 tests. I was trying to convert L to boolean and then use andlist on it to check for falses
  ;but this didnt work.
)
...
(define (exactly_one_q L)
  ...
  (if (null? L)
      #f
      (if (equal? (first L) 'q)
          (if (exactly_one_q (rest L)) #f #t)
          (if (exactly_one_q (rest L)) #t #f)))
)
...
(define (odd_amt_q L)
    (if (null? L)
      #f
      (if (equal? (first L) 'q)
          (if (odd_amt_q (rest L)) #f #t)
          (if (odd_amt_q (rest L)) #t #f))) ;Reused the code for 13 and it worked perfectly.
)
...
(define (odd_amt_q_v2 L)
  (foldl xori #f (map (lambda (x) (equal? (first L) 'q)) L))
)
