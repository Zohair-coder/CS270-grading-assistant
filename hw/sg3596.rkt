...
(define (all_q L)
  (if (null? L) 0 (andi (equal? 'q (first L)) (all_q (rest L))))
)
...
(define (all_q_v2 L)
  (if (equal? (foldl (λ (x y) (if (equal? x y) 'q #f)) 'q L) 'q) #t #f)
)
...
(define (at_least_one_q L)
  (if (null? L) #f (ori (equal? 'q (first L)) (at_least_one_q (rest L))))
)
...
(define (at_least_one_q_v2 L)
  (foldl (λ (x y) (if (equal? x 'q) #t #f)) 'q L) 
)
...
(define (exactly_one_q L)
  (if (null? L) #f (xori (equal? 'q (first L)) (exactly_one_q (rest L))))
)

#|
THIS WAS AN HONEST ATTEMPT TO CLOSE THE GAPS USING MY INITIAL ANSWER BUT IT IS INEFFICIENT, OVERCOMPLICATED AND MOST OF ALL, IT DOES NOT WORK.
(define (exactly_one_q L)
  (if (null? L) #f (xori (if (implies (andi (not (null? (rest L))) (equal? 'q (second L))) (equal? 'q (first L))) #f #t) (exactly_one_q (rest L)))
))
|#
...
(define (odd_amt_q L)
  (if (null? L) #f (xori (equal? 'q (first L)) (odd_amt_q (rest L)))) ;i believed this was the answer to q13, but i realized it suited this alternating t/f case perfectly
)
...
(define (odd_amt_q_v2 L)
  (foldl (λ (y z) (xori y z)) #f (map (λ (x) (equal? 'q x)) L))
)
