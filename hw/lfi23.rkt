...
(define (andlist L)
  (if (empty? L) #t (foldr (λ (x y) (and x y)) #t L))
)
...
(define (orlist L)
  (if (empty? L) #f (foldr (λ (x y) (or x y)) #f L))
)
...
(define (all_q L)
  (if (empty? L) #f (all_q_work L))
 )

; Helper function
(define (all_q_work L)
  (if (empty? L) #t (if (equal? (first L) 'q) (all_q_work (rest L)) #f))
)
...
(define (all_q_v2 L)
  (if (empty? L) #f (andlist (map (λ (x) (if (equal? x 'q) #t #f)) L)))
)
...
(define (at_least_one_q_v2 L)
 (if (empty? L) #f
     (orlist (map (λ (x) (if (equal? x 'q) #t #f)) L)))
)
...
(define (exactly_one_q L)
  (if (null? L) #f
   (and (at_least_one_q L) (null? (rest (foldr (λ (x y) (if (equal? x 'q) (cons x y) y)) null L))))) 
)
...
(define (odd_amt_q L)
  (cond
    [(empty? L) #f]
    [(equal? 'q (first L)) (not (odd_amt_q (rest L)))]
...
(define (odd_amt_q_v2 L)
  (if (null? L) #f
     (foldr xori #f (map (λ (x) (if (equal? x 'q) #t #f)) L)))
)
