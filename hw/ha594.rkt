...
(define (all_q L)
 (cond
  ((null? L) #t)
  (else (andi (equal? (first L) 'q) (all_q (rest L))))
 )
)
...
(define (all_q_v2 L)
(if (equal? 'q (foldl (lambda (x y) (if (andi (eq? x 'q) (eq? y 'q)) 'q 'x)) 'q L)) #t #f))
...
(define (at_least_one_q L)
 (cond
  ((null? L) #f)
  (else (or (equal? (first L) 'q) (at_least_one_q (rest L))))
 )
)
...
(if (equal? 'q (foldl (lambda (x y) (if (ori (eq? x 'q) (eq? y 'q)) 'q 'x)) 'x L)) #t #f))
...
(define (exactly_one_q L)
 (cond
 ((null? L) #f)
 ((equal? (first L) 'q)  (if (equal? (at_least_one_q_v2 (rest L)) #f) #t #f))
...
(cond
((null? L) #f)
 ((equal? (first L) 'q)  (if (equal? (odd_amt_q (rest L)) #f) #t #f))
...
(if (equal? 'q (foldl (lambda (x y) (if (xori (eq? x 'q) (eq? y 'q)) 'q 'x)) 'x L)) #t #f))
