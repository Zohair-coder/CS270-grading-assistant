...
(define (all_q L)
  (if (null? L) #f (if (equal? (first L) 'q) (if (null? (rest L)) #t (all_q (rest L))) (null? L)))
)
...
(define (all_q_v2 L)
  (if (null? L) #f (andlist (map (λ (x) (equal? x 'q)) L)))
)
...
(define (at_least_one_q L)
  (if (null? L) #f (if (equal? (first L) 'q) #t (if (null? (rest L)) #f (at_least_one_q (rest L)))))
)
...
(define (at_least_one_q_v2 L)
  (if (null? L) #f (orlist (map (λ (x) (equal? x 'q)) L)))
)
...
;Question 13
; Input:  L is a list of symbols (possibly empty).
; Output: (exactly_one_q L) is true if and only if L contains exactly one q. (ASK IF YOU COULD CALL A PAST ANSWER)
; Hint: The answer to question 11 is helpful to use here.
(define (exactly_one_q L)
  (if (null? L) #f (if (equal? (first L) 'q) (if (null? (rest L)) #t (if (at_least_one_q (rest L)) #f #t)) (if (null? (rest L)) #f (exactly_one_q (rest L)))))
)
...
(define (odd_amt_q L)
  (if (null? L) #f (if (equal? (first L) 'q) (xori #t (odd_amt_q (rest L))) (xori #f (odd_amt_q (rest L)))))
)
...
(define (odd_amt_q_v2 L)
  (if (null? L) #f (foldr xori #f (map (λ (x) (equal? x 'q)) L)))
)
