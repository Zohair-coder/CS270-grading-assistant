...
(define (andlist L)
  (if (zero? (foldl (lambda (x y) (+ y (if x 0 1))) 0 L)) #t #f)
)
...
(define (orlist L)
  (if (zero? (foldl (lambda (x y) (+ y (if x 1 0))) 0 L)) #f #t)
)
...
Why is this failing on successful tests :(
Unbound identifier in q

had to use ' in front of q, thank you prof!

|#

(define (all_q L)
  (if (null? L) #t (if (equal? (first L) 'q) (all_q (rest L)) #f))
)
...
(define (all_q_v2 L)
  (if (zero? (foldl (lambda (x y) (+ y (if (equal? x 'q) 0 1))) 0 L)) #t #f)
)
...
(define (at_least_one_q_v2 L)
  (if (zero? (foldl (lambda (x y) (+ y (if (equal? x 'q) 1 0))) 0 L)) #f #t)
)
...
; Not sure how to avoid using integers here?
; This is my best attempt, works on 8/10

(define (exactly_one_q L)
  (cond
    [(null? L) #f]
    [(andi (equal? 'q (first L)) (exactly_one_q (rest L))) #f]
    [(xori (equal? 'q (first L)) (exactly_one_q (rest L))) #t]
    [else #f]
    )
)
...
(define (odd_amt_q L)
  (if (null? L) #f (xori (equal? 'q (first L)) (odd_amt_q (rest L))))
)
...
(define (odd_amt_q_v2 L)
  (if (equal? #t (foldl (lambda (x y) (if (equal? x 'q) (noti y) y)) #f L)) #t #f)
)
