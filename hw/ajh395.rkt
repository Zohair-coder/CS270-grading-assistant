...
(define (all_q L)
  (if (null? L) #t (if (not (q? (first L))) #f (all_q (rest L))))
)
(define (q? a)
(if (equal? 'q a) #t #f)
  )
...
(define (all_q_v2 L)
  (andlist (map (lambda (x) (equal? x 'q)) L))
)
...
(define (at_least_one_q L)
  (if (null? L) #f (if (q? (first L)) #t (at_least_one_q (rest L))))
)
...
(define (at_least_one_q_v2 L)
  (orlist (map (lambda (x) (equal? x 'q)) L))
)
...
(define (exactly_one_q L)
  (if (null? L) #f (if (equal? 'q (first L)) (if (at_least_one_q (rest L)) #f #t) (exactly_one_q (rest L))))
)
...
(define (odd_amt_q_v2 L)
  (foldr xori #f (map (lambda (x) (equal? 'q x)) L))
)
