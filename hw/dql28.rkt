...
(define (all_q L)
  (cond
    [ (null? L) #t]
    [ else (if (equal? (first L) 'q) (all_q (rest L)) #f)]))
...
(define (at_least_one_q L)
  (cond
    [ (null? L) #f]
    [ else (if (equal? (first L) 'q) #t (at_least_one_q (rest L)))]))
...
(define (exactly_one_q L)
  (cond
    [ (null? L) #f]
    [ else (if (equal? (first L) 'q) (not (at_least_one_q_v2 (rest L))) (exactly_one_q (rest L)))]))
...
(define (odd_amt_q L [x #f])
  (cond
    [(null? L) x]
    [(equal? (first L) 'q) (odd_amt_q (rest L) (noti x))]
    [else (odd_amt_q (rest L) x)]))
...
(define (odd_amt_q_v2 L)
  (foldl (λ (x y) (xor x y)) #f (map (λ (x) (equal? x 'q)) L)))
