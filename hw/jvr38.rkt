...
(define (xori a b)
  (if a (if b #f #t) (if b #t #f))); ANSWER 4
...
(define (iffi a b)
  (if a (if b #t #f) (if b #f #t))); ANSWER 6
...
(define (andlist L)
  (if (foldr andi #t L) #t #f)) ; ANSWER 7
...
(define (all_q L)
  (if (if (null? L) #t (andi (equal? 'q (first L)) (all_q (rest L)))) #t #f)) ; ANSWER 9
...
(define (all_q_v2 L)
  (if (andlist (map (λ (a) (equal? a 'q)) L)) #t #f)) ; ANSWER 10
...
(define (at_least_one_q L)
  (if (if (null? L) #f (ori (equal? 'q (first L)) (at_least_one_q (rest L)))) #t #f)) ; ANSWER 11
...
(define (at_least_one_q_v2 L)
  (if (orlist (map (λ (a) (equal? a 'q)) L)) #t #f)) ; ANSWER 12
...
(define (exactly_one_q L)
  (if (if (null? L) #f (if (equal? 'q (first L)) (if (orlist (map (λ (a) (if (equal? 'q a) #t #f)) (rest L))) #f #t) (exactly_one_q (rest L)))) #t #f)); ANSWER 13
...
(define (odd_amt_q L)
  (if (if (null? L) #f (xori (equal? 'q (first L)) (odd_amt_q (rest L)))) #t #f)) ; ANSWER 14
...
(define (odd_amt_q_v2 L)
  (if (foldr xori #f (map (λ (a) (if (equal? a 'q) #t #f)) L)) #t #f)) ; ANSWER 15
