...
(define (all_q L)
  (if (empty? L) #t (if (equal? 'q (first L)) (all_q (rest L)) #f))
)
...
(define (at_least_one_q L)
  (if (empty? L) #f (if (equal? 'q (first L)) #t (at_least_one_q (rest L))))
)
...
(define (exactly_one_q L)
  (if (empty? L) #f (if (equal? 'q (first L)) (if (at_least_one_q (rest L)) #f #t) (exactly_one_q (rest L))))
)
...
(define (odd_amt_q L)
  (if (empty? L) #f (if (equal? 'q (first L)) (if (odd_amt_q (rest L)) #f #t) (odd_amt_q (rest L))))
)
...
(define (odd_amt_q_v2 L)
  (foldr (lambda (a b) (if (equal? 'q a) (if b #f #t) b)) #f L)
)
