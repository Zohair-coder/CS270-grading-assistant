...
(define (all_q L)
 (if (null? L) #t (if (equal? 'q (first L)) (all_q (rest L)) #f))
)
...
(define (all_q_v2 L)
  (foldr andi #t (map (lambda (x) (equal? 'q x)) L))
)
...
(define (exactly_one_q L)
  (cond [(empty? L) #f]
        [(equal? 'q (first L))(noti(at_least_one_q (rest L)))]
...
(define (odd_amt_q L)
  (if (empty? L) #f (if (equal? (first L) 'q) (if (odd_amt_q (rest L)) #f #t) (if (odd_amt_q (rest L)) #t #f)))
)
