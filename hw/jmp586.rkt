...
(define (all_q L)
  (cond
    [(null? L) #f]
    [(null? (rest L)) (equal? 'q (first L))]
    [(equal? 'q (first L)) (all_q (rest L))]
    [else #f]
   )
)
...
(define (all_q_v2 L)
  (andlist (map (lambda (x) (equal? 'q x)) L))
)
...
(define (at_least_one_q_v2 L)
  (orlist (map (lambda (x) (equal? 'q x)) L))
)
...
(define (exactly_one_q L)
 (cond
   [(null? L) #f]
   [(equal? (first L) 'q) (noti (at_least_one_q_v2 (rest L)))]
...
;if first of L q then we have odd q's #t
;if next L is q then we even q's #f
(define (odd_amt_q L)
  (cond
   [(null? L) #f]
...
;fold with xor starting with a base of #f since 0 is even.
;if the first item is q then we have one q/#f then if next item is q (xori #t #t) then we have even q and xor will continue to be false
;until another q is encountered
(define (odd_amt_q_v2 L)
  (foldl xori #f (map (lambda (x) (equal? 'q x)) L))
)
